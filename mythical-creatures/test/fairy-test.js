...
describe('Fairy', () => {
  ...
  it('should turn flowers into more dresses', () => {
    ...
    fairy.makeDresses(['Daffodil', 'Tulip', 'Poppy', 'Lilac', 'Rose']);

    assert.deepEqual(fairy.clothes.dresses, ['Iris', 'Daffodil', 'Tulip', 'Poppy', 'Lilac', 'Rose']);
  });
  ...
  it('should not raise stolen infants as its own if it is not vengeful', () => {
    const fairy = new Fairy('Winnie');
    const firstInfant = { name: 'Mary', eyes: 'Green', disposition: 'Calm' };
    const secondInfant = { name: 'Clarke', eyes: 'Brown', disposition: 'Gentle'};
